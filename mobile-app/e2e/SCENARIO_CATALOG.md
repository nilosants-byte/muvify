...
|---|---|---|---|---|
| GA-001 | App abre e chega no login | HIGH | AUTOMATIZADO | `e2e/smoke/app-launch-smoke.yaml` |
| GA-002 | Splash de lancamento durante boot | MEDIUM | MAPEADO | - |
| GA-003 | Bloqueio offline antes de autenticar | HIGH | MAPEADO | - |
| GA-004 | Login com sucesso como CLIENT | HIGH | AUTOMATIZADO | `e2e/auth/login-success-client.yaml` |
| GA-005 | Login com sucesso como PROVIDER | HIGH | AUTOMATIZADO | `e2e/auth/login-success-provider.yaml` |
| GA-006 | Login com sucesso como ADMIN | HIGH | MAPEADO | - |
| GA-007 | Login com campos obrigatorios vazios | HIGH | AUTOMATIZADO | `e2e/auth/login-required-fields-negative.yaml` |
| GA-008 | Login com credenciais invalidas | HIGH | MAPEADO | - |
| GA-009 | Lembrar e-mail no login persiste apos reabrir | MEDIUM | MAPEADO | - |
| GA-010 | Navegacao Login -> Cadastro -> Login | MEDIUM | MAPEADO | - |
| GA-011 | Cadastro CLIENT com sucesso | HIGH | AUTOMATIZADO | `e2e/auth/register-success-client.yaml` |
| GA-012 | Cadastro PROVIDER com sucesso | HIGH | MAPEADO | - |
| GA-013 | Cadastro com campos obrigatorios vazios | HIGH | AUTOMATIZADO | `e2e/auth/register-required-fields-negative.yaml` |
| GA-014 | Cadastro com email ja existente | HIGH | MAPEADO | - |
| GA-015 | Acesso a tela de recuperar senha | MEDIUM | AUTOMATIZADO | `e2e/auth/forgot-password-navigation.yaml` |
| GA-016 | Recuperar senha sem e-mail (validacao) | MEDIUM | AUTOMATIZADO | `e2e/auth/reset-password-validation-negative.yaml` |
...
| GA-023 | Logout retorna para login | HIGH | MAPEADO | - |
...
| ID | Cenario | Prioridade | Status | Flow atual |
|---|---|---|---|---|
| CL-001 | Home cliente renderiza mapa e cards principais | HIGH | AUTOMATIZADO | `e2e/auth/login-success-client.yaml` |
...
| CL-020 | CreateBooking sucesso com oferta padrao | HIGH | MAPEADO | - |
| CL-021 | CreateBooking sucesso com oferta promocional | HIGH | MAPEADO | - |
| CL-022 | BookingConfirmation exibe id e dados do agendamento | HIGH | MAPEADO | - |
| CL-023 | BookingPaymentStatus cartao (autorizado/capturado) | HIGH | MAPEADO | - |
| CL-024 | BookingPaymentStatus pix (geracao qr/copia e cola) | HIGH | MAPEADO | - |
| CL-025 | ClientBookings lista agenda do cliente | HIGH | AUTOMATIZADO | `e2e/scheduling/client-booking-detail-smoke.yaml` |
| CL-026 | ClientBookingDetail abre dados de status/pagamento | HIGH | AUTOMATIZADO | `e2e/scheduling/client-booking-detail-smoke.yaml` |
| CL-027 | ClientBookingDetail atualiza codigo/QR de presenca | HIGH | MAPEADO | - |
...
| CL-045 | ClientProfile editar nome e persistir | MEDIUM | AUTOMATIZADO | `e2e/profile/client-profile-smoke.yaml` |
...
| CL-056 | Navegacao principal cliente (Home/Promo/Treino/Perfil/Agenda) | HIGH | AUTOMATIZADO | `e2e/regression/client-main-navigation.yaml` |

## 3) Fluxos Logado - Personal Trainer (PR-*)

| ID | Cenario | Prioridade | Status | Flow atual |
|---|---|---|---|---|
| PR-001 | Home do profissional carrega indicadores e atalhos | HIGH | AUTOMATIZADO | `e2e/professional/provider-main-navigation-smoke.yaml` |
...
| PR-010 | ProfessionalAgenda abre e lista atendimentos | HIGH | AUTOMATIZADO | `e2e/professional/provider-main-navigation-smoke.yaml` |
...
| PR-015 | BookingDetailProfessional exibe dados completos | HIGH | MAPEADO | - |
...
| PR-018 | ProfessionalConfirmCompletion exige selfie de prova | HIGH | MAPEADO | - |
| PR-019 | ProfessionalConfirmCompletion conclui com selfie valida | HIGH | MAPEADO | - |
...
| PR-022 | ProfessionalConsultancyCenter abre e mostra painel | HIGH | AUTOMATIZADO | `e2e/professional/provider-main-navigation-smoke.yaml` |
...
| PR-029 | ProfessionalStudents abre dashboard por servico | HIGH | AUTOMATIZADO | `e2e/professional/provider-main-navigation-smoke.yaml` |
...
| PR-038 | ProfessionalProfileEditor salvar dados obrigatorios | HIGH | MAPEADO | - |
...
| PR-051 | Navegacao principal profissional (Home/Agenda/Consultoria/Alunos) | HIGH | AUTOMATIZADO | `e2e/professional/provider-main-navigation-smoke.yaml` |
...
| ID | Cenario | Prioridade | Status | Flow atual |
|---|---|---|---|---|
| AD-001 | Login admin com sucesso | HIGH | MAPEADO | - |
| AD-002 | Usuario sem role ADMIN nao acessa stack admin | HIGH | MAPEADO | - |
| AD-003 | AdminHome carrega overview e rankings | HIGH | MAPEADO | - |
| AD-004 | AdminHome troca mes e atualiza metricas | MEDIUM | MAPEADO | - |
| AD-005 | AdminCrefValidation lista fila pendente/aprovada/reprovada | HIGH | MAPEADO | - |
...
## Resumo de Uso em Lotes

- Lote `HIGH`: cenarios bloqueantes de negocio (auth, mapa/raio, booking, pagamentos, operacao principal provider/admin, sessao/offline).
- Lote `MEDIUM`: fluxos importantes de operacao e UX secundaria.
- Lote `LOW`: cenarios de refinamento, robustez e qualidade complementar.

Execucao atual em lotes de flows ja implementados:
